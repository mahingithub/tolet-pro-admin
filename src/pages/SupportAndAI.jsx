...
const SupportAndAI = () => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      await logAuditAction({ action: 'support.reply', targetType: 'ticket', targetId: activeId });
      setReplyText('');
    } catch (err) {
  ...
  const handleResolve = async () => {
    ...
    await logAuditAction({ action: 'support.resolve', targetType: 'ticket', targetId: activeId });
  };

  const handleReopen = async () => {
    if (!activeId) return;
    await reopenTicket(activeId);
    await logAuditAction({ action: 'support.reopen', targetType: 'ticket', targetId: activeId });
  };
    ...
    <div className="max-w-[1400px] mx-auto pt-4 pb-12 h-[calc(100vh-100px)] flex flex-col">
      ...
      <div className="grid grid-cols-12 gap-6 flex-1 min-h-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      ...
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                        {activeTicket.ticket.userName} · {activeTicket.ticket.userPhone}
                      </span>
                    </div>
                  </div>
        ...
        <aside className="col-span-12 md:col-span-3 space-y-4 overflow-y-auto custom-scrollbar min-h-0 pb-2">
          ...
          {activeTicket && (
            <UserContextCard ctx={activeTicket.userContext} ticket={activeTicket.ticket} />
          )}
        </aside>
      </div>
      )}
  ...
  <>
    <div className="bg-white rounded-[2rem] p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
      ...
      <div className="space-y-2 text-[11px] font-bold text-gray-600">
        ...
        />
        <Line icon={Inbox} label={`${ctx.resolvedTicketCount} resolved past tickets`} />
        {ticket.origin?.source === 'ai_widget' && <Line icon={Bot} label="Came from AI handoff" />}
      </div>
    </div>
